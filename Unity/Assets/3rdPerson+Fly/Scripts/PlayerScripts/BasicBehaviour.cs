...
public class BasicBehaviour : MonoBehaviour
{
	...
	private float moveFromMax = 0f;
	private float moveToSide = 0f;

	public OSCReceiver oscReceiver;
	public string address = "/Player/move/forward";
	public string address2 = "/Player/move/horizontal";
    ...
    private void Start()
    {
		...
		oscReceiver.Bind(address2, ReceivedHorizontal);
	...
	/*********************************************************/
	//callback for OSC messages
	private void ReceivedHorizontal(OSCMessage message)
	{
		if (message.ToFloat(out float value))
		{
			//Debug.Log($"from Horizontal: {value}");
	...
	void Update()
	{
		...
		h = Input.GetAxis("Horizontal") + moveToSide;
		v = Input.GetAxis("Vertical") + moveFromMax;
